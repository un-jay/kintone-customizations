...
- GASの実行時間上限超過等で「送信処理中」のままスタックした行は、次回実行時に自動検出し管理者へ通知する（追加のAPIリクエスト無し）
- 送信タイミングは`setupTrigger`を何分に実行したかに関わらず、毎時00分/05分等のキリの良い時刻に自動で揃う（詳細は[gas/README.md](./gas/README.md)参照）
...
上記の図には表れないが、GASの実行時間上限超過等で行が「送信処理中」のまま無言でスタックした場合や、レコード取得自体が失敗するようなシステム全体のエラーは、`ADMIN_NOTIFY_EMAIL`宛のメールで管理者に通知される（詳細は[CLAUDE.md](./CLAUDE.md#エラーの気付き方個々の行のエラー-vs-管理者通知メール)参照）。
